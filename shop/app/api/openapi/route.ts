...
export function GET() {
  const spec = {
    ...
    info: {
      ...
      version: "1.0.0",
      description:
        "Demo commerce API for customer-facing AI agents. Catalog is public; cart/checkout/orders/returns require a customer bearer token (each seeded customer has one).",
    },
    servers: [{ url: siteUrl() }],
    components: {
      securitySchemes: {
        customerToken: {
          ...
          scheme: "bearer",
          description:
            "Per-customer API token. Seeded demo tokens: shop_tok_ava, shop_tok_marcus, shop_tok_priya.",
        },
      },
    },
    ...
    paths: {
              ...
              "application/json": {
                schema: {
                  ...
                  properties: {
                    ...
                    size: { type: "string", description: "Required when the product has sizes (see product.availableSizes)" },
      ...
      "/api/checkout": {
        post: {
          ...
          responses: { "200": { description: "{ checkoutUrl, orderId, orderNumber }" }, "400": { description: "Empty cart" }, "501": { description: "Stripe not configured" } },
        },
      },
      "/api/checkout/confirm": {
        post: {
          summary: "Reconcile a Checkout Session and finalize the order if paid",
      ...
      "/api/orders/{id}": {
        get: {
          summary: "Get an order by UUID or order number (LMN-…)",
          parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }],
