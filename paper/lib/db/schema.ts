...
export const products = pgTable("products", {
  ...
  /* ── Paper attributes (repurposed generic storefront — what the agent reasons over) ── */
  brand: text("brand").notNull().default(""), // product line: Dunder Mifflin | Astrobright | Schrute Farms…
  gender: text("gender").notNull().default("unisex"), // unused for paper
  material: text("material").notNull().default(""), // paper stock / weight
  colorway: text("colorway").notNull().default(""), // paper color
  useCase: text("use_case").notNull().default(""), // category: copy | cardstock | photo | specialty | envelopes | supplies
  /** Printer / usage guidance, e.g. "Jam-free in laser & inkjet". */
  fit: text("fit").notNull().default(""),
  /** Available paper sizes / formats in display order, e.g. ["Letter","Legal","A4"]. */
  sizes: jsonb("sizes").$type<string[]>().notNull().default([]),
  /** Per-format on-hand inventory, e.g. {"Letter": 40, "Legal": 0}. */
  sizeStock: jsonb("size_stock").$type<Record<string, number>>().notNull().default({}),
  /** Misc spec sheet (weight, brightness, sheets/ream, etc.). */
