...
export function PayButton({ policyNumber }: { policyNumber: string }) {
  ...
  async function pay() {
    setBusy(true);
    setError(null);
    const res = await fetch(`/api/policies/${policyNumber}/pay`, { method: "POST" });
    const body = await res.json().catch(() => ({}));
    setBusy(false);
    if (res.ok) { setDone(true); router.refresh(); }
    else setError(body.error ?? "Payment failed");
  }
  ...
  return (
    <div className="flex flex-col items-start gap-1">
      <button onClick={pay} disabled={busy} className="inline-flex items-center gap-2 rounded-full bg-accent px-6 py-2.5 text-sm font-medium text-accent-foreground hover:opacity-90 disabled:opacity-50">
        {busy && <Loader2 className="h-4 w-4 animate-spin" />} Pay premium
      </button>
